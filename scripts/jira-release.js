...
const JIRA_TOKEN =
  ...
  process.env.SEMANTIC_RELEASE_JIRA_API_TOKEN;

const PROJECT_KEY = process.env.JIRA_PROJECT_KEY || "DRNT2";
const VERSION_PREFIX = process.env.JIRA_VERSION_PREFIX || "release-note-tester-v";
...
async function jiraRequest(endpoint, options = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Jira API error [${response.status} ${response.statusText}] at ${endpoint}: ${errorText}`);
  }
...
async function run() {
  ...
  if (!rawVersion) {
    console.error("❌ Error: Release version argument is required (e.g. node jira-release.js 1.7.0)");
    process.exit(1);
  }
  ...
  if (!JIRA_EMAIL || !JIRA_TOKEN) {
    console.warn("⚠️  Jira credentials not provided (JIRA_USER_EMAIL / JIRA_API_TOKEN). Skipping Jira release creation.");
    return;
  }

  try {
    // 1. Fetch Project Details
    console.log(`🔍 [Jira Release] Fetching project details for ${PROJECT_KEY}...`);
    const project = await jiraRequest(`/rest/api/3/project/${PROJECT_KEY}`);
    console.log(`✅ [Jira Release] Connected to project: ${project.name} (ID: ${project.id})`);

    // 2. Check or Create Version
    console.log(`🔍 [Jira Release] Checking if version "${versionName}" exists...`);
    const versions = await jiraRequest(`/rest/api/3/project/${PROJECT_KEY}/versions`);
    let targetVersion = versions.find((v) => v.name === versionName);

    if (targetVersion) {
      console.log(`ℹ️  [Jira Release] Version "${versionName}" already exists (ID: ${targetVersion.id}).`);
    } else {
      ...
      console.log(`🎉 [Jira Release] Created Jira Version "${versionName}" (ID: ${targetVersion.id})`);
    }

    // 3. Extract Tickets and Associate with Version
    const tickets = getTicketsFromGit();
    console.log(`📋 [Jira Release] Found ${tickets.length} ticket(s) in release commits: ${tickets.join(", ") || "(none)"}`);
    ...
    for (const ticketKey of tickets) {
      try {
        ...
        const issue = await jiraRequest(`/rest/api/3/issue/${ticketKey}?fields=fixVersions,status`);
        const existingFixVersions = issue.fields?.fixVersions || [];

        // Check if version is already attached
        const hasVersion = existingFixVersions.some((v) => v.id === targetVersion.id || v.name === versionName);
        if (!hasVersion) {
        ...
        if (currentStatus !== "Released" && currentStatus !== "Done") {
          ...
          if (targetTransition) {
            console.log(`  🚀 Transitioning ${ticketKey} from "${currentStatus}" to "${targetTransition.to.name}"...`);
            ...
            console.log(`  ⚠️  No valid transition to "Released" or "Done" found for ${ticketKey}. Current status: "${currentStatus}"`);
          }
        } else {
    ...
    console.error(`\n❌ [Jira Release] Error processing Jira release:`, err.message);
